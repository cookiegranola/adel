...
irr::video::SMaterial PostProcess::materialPP;
s32 PostProcess::shadowShader;
...
void PostProcess::InitShaders()
{
	...
	std::string vertex_path = getShaderPath("postprocess", "shadow_vertex.glsl");
	std::string pixel_path = getShaderPath("postprocess", "shadow_fragment.glsl");
	std::string vertex_program = readFile(vertex_path);
	std::string pixel_program = readFile(pixel_path);

	PostProcess::shadowShader = postProcess.driver->getGPUProgrammingServices()->addHighLevelShaderMaterial(
		vertex_program.c_str(), "main", irr::video::EVST_VS_2_0,
		pixel_program.c_str(), "main", irr::video::EPST_PS_2_0);
